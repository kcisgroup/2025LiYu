...
def val(model, epoch, save_path, writer):
    ...
    with torch.no_grad():
        mae_sum = 0
        test_loader = test_dataset(image_root=opt.test_path + '/COD/Image/',
                                  gt_root=opt.test_path + '/COD/GT/',
                                  testsize=opt.trainsize)
        # test_loader = test_dataset(image_root=opt.test_path + '/COD/Image/',
        #                            gt_root=opt.test_path + '/COD/GT/',
        #                            depth_root=opt.test_path + '/COD/Depth/',
        #                            testsize=opt.trainsize)
...
def train(train_loader, model, optimizer, epoch, test_path):
    ...
    for i, pack in enumerate(train_loader, start=1):
        for rate in size_rates:
            ...
            # loss_P2 = LogitsBCE(P2, gts)
            loss_P2 = structure_loss(P2, gts)

            # loss = loss_p1 + loss_P2
            loss = loss_p1 + loss_P2 + bce_loss(edge, edges)
            # ---- backward ----
            loss.backward()
    ...
    if epoch % opt.epoch_save == 0:
        torch.save(model.state_dict(), save_path + str(epoch) + 'TPnet.pth')


if __name__ == '__main__':

    ##################model_name#############################
    model_name = 'TPnet_pvt_hiy'
    ...
    parser.add_argument('--load', type=str, default='./Dataset/res/0.0274/TPnet.pth', help='train from checkpoints')
